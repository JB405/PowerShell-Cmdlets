function Get-LifeNumber {
   ...
   $table = (
    ...
    if ($Raw.IsPresent){ $result }
    else{ return "Your Life Number is $lifenumber, which represents '$($result.Type)'. The key values associated with this number are: $($result.values)." }
}
